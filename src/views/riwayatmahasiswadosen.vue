<template>
  ...
  const fetchData = async () => {
	try {
	  ...
	  // Ambil data setoran
	  const setoranResponse = await axios.get(`https://samatif.xyz/setoran/by-nim.php?nim=${nim.value}`, config);
	  setoranList.value = setoranResponse.data.setoran;
  
	  // Ambil data persentase skills
	  const skillsResponse = await axios.get(`https://samatif.xyz/setoran/sudahbelum.php?nim=${nim.value}`, config);
  ...
  const deleteSetoran = async (id_setoran) => {
	...
	try {
	  ...
	  // Hapus setoran
	  const response = await axios.post('https://samatif.xyz/setoran/delete.php', formData, config);
